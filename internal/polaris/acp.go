...
func (a *acpSession) handleToolUpdate(update json.RawMessage) {
	...
	a.mergeTool(u.ToolCallID, name, acpToolDetail(acpDecodeInput(u.RawInput)))

	// Trailing space after the id matters: git.ExtractLogFilePaths (which counts
	// modified files) only matches `→ [#id] Edit · path` with whitespace after `]`.
	tag := ""
	if u.ToolCallID != "" {
		tag = "[#" + u.ToolCallID + "] "
	}
	...
	case "in_progress":
		a.emitToolCall(u.ToolCallID, tag)
	case "completed", "failed":
		a.emitToolCall(u.ToolCallID, tag)
		// Build the display text from `content`. Edit tools (search_replace/
		// write_file) send structured `type:"diff"` blocks with old/new text, which
		// we render as a -/+ diff the log panel colourises. Other tools send plain
		// text blocks.
		...
		// vibe-acp (which sets `_meta.tool_name`) puts a short recap in `content`
		// for read-like tools and the real payload in a JSON `rawOutput` model, so
		// we dig the payload out of there — unless `content` already holds a diff.
		// opencode puts the display text directly in `rawOutput`, content as fallback.
		var out string
		switch {
		case hasDiff:
			out = content.String()
		case u.Meta.ToolName != "":
			...
			// Edit tools that don't send structured diff content leave SEARCH/REPLACE
			// blocks in the result text; render them as a -/+ diff like Claude.
		...
		if out = strings.TrimSpace(out); out != "" {
			// Both success (`← `) and tagged error (`✗ [#id]`) results open an
			// absorbing block in the log renderer, so multi-line command output
			// attaches to the tool call instead of spilling into orphan text.
			prefix := "← "
			if u.Status == "failed" {
				prefix = "✗ "
			}
			a.emit(prefix + tag + capLines(out, 200))
		}
	}
}
...
// mergeTool records the tool's name/detail, keeping whatever a prior event
// already supplied (later updates may omit them).
func (a *acpSession) mergeTool(id, name, detail string) {
	a.mu.Lock()
	defer a.mu.Unlock()
	st := a.toolMap()[id]
	if st == nil {
...
// emitToolCall writes the `→ Name · detail` line once per tool call.
func (a *acpSession) emitToolCall(id, tag string) {
	a.mu.Lock()
	st := a.toolMap()[id]
	if st == nil {
		st = &acpToolState{}
...
// curMsg/curThought are written by the readLoop goroutine and drained by
// runTurn (a different goroutine), so all access goes through mu; emit() runs
// outside the lock to avoid holding it across log I/O.
...
// appendMsg accumulates streamed assistant text and emits it line by line as
// complete lines arrive, keeping only the trailing partial line buffered. The
// log renderer merges consecutive text lines into one block, so this streams the
// response progressively instead of dumping the whole turn at flush time.
...
// emitMsgLines writes each complete assistant line, preserving blank lines so
// markdown paragraph breaks survive (emit() trims, which would merge paragraphs
// into run-on text). Blank lines are written directly as empty log lines.
func (a *acpSession) emitMsgLines(s string) {
	for _, ln := range strings.Split(s, "\n") {
		if strings.TrimSpace(ln) == "" {
			_ = a.svc.appendAgentLog(a.agentID, "")
		} else {
			a.emit(ln)
		}
	}
...
func (a *acpSession) emit(s string) {
	if s = strings.TrimSpace(s); s == "" {
		return
	}

	stamped := fmt.Sprintf("[%s] %s", time.Now().Format("15:04:05"), s)
	_ = a.svc.appendAgentLog(a.agentID, stamped)
}
