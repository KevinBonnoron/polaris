		...
		case "user":
			...
			case string:
				if c = strings.TrimSpace(c); c != "" && !strings.HasPrefix(c, "<ide_opened_file>") {
					_ = service.appendAgentLog(agentID, fmt.Sprintf("[%s] > %s", ts, c))
				}
			case []any:
				isToolResult := false
				for _, item := range c {
				...
				if isToolResult {
					for _, line := range renderUserToolResults(raw, toolInputs) {
						_ = service.appendAgentLog(agentID, fmt.Sprintf("[%s] %s", ts, line))
					}
				} else {
					var parts []string
					for _, item := range c {
					...
					if text := strings.TrimSpace(strings.Join(parts, "\n")); text != "" {
						_ = service.appendAgentLog(agentID, fmt.Sprintf("[%s] > %s", ts, text))
					}
				}
			}
		case "assistant":
			files := make(map[string]struct{})
			for _, line := range renderAssistant(raw, files, toolInputs, nil, &stats) {
				_ = service.appendAgentLog(agentID, fmt.Sprintf("[%s] %s", ts, line))
			}
		}
	}
