...
func (service *Service) Spawn(in SpawnAgentInput) (*Agent, error) {
	...
	if isACP {
		_ = service.appendAgentLog(created.ID, startMsg)
	...
	go service.runner.run(service, created.ID, in.Kind, binary, args, runDir, startMsg, initialInput, false, nil)
	return &created, nil
}
...
func (service *Service) RecoverInterruptedAgents() error {
	...
	for _, a := range stale {
		...
		if a.PendingQuestion != nil {
			_ = service.appendAgentLog(a.ID, fmt.Sprintf("[%s] (waiting for your answer after app restart)", time.Now().Format("15:04:05")))
		...
		_ = service.appendAgentLog(a.ID, fmt.Sprintf("[%s] (interrupted by app restart)", time.Now().Format("15:04:05")))
...
func (service *Service) autoResumeAgent(a Agent) error {
	...
	_ = service.appendAgentLog(a.ID, stamped)
	workDir := agentRunDir(&a, project)
	initialInput := ""
	if a.Kind == "claude-code" {
		initialInput = resumePrompt
	}
	go service.runner.run(service, a.ID, a.Kind, binary, args, workDir, "", initialInput, true, env)
	return nil
}
...
func (service *Service) Send(agentID, message string) error {
	...
	if agent.Kind == "opencode" || agent.Kind == "mistral" {
		...
		_ = service.appendAgentLog(agentID, stamped)
		_ = service.store.PatchAgent(agentID, map[string]any{"status": "working"})
		a.prompt(service.runner, message)
		return nil
	}

	// Interactive CLIs (copilot/codex): a follow-up message is a reply piped to
	// the live subprocess on stdin. The process must still be running (we don't
	// restart those sessions from scratch). claude-code and opencode instead
	// resume a finished session with a fresh turn, so they skip this branch.
	if agent.Kind != "claude-code" && agent.Kind != "opencode" {
		if !service.runner.isRunning(agentID) {
			return fmt.Errorf("agent %q is no longer running", agentID)
		}
		stamped := fmt.Sprintf("[%s] > %s", time.Now().Format("15:04:05"), message)
		if err := service.appendAgentLog(agentID, stamped); err != nil {
	...
	go service.runner.run(service, agentID, agent.Kind, binary, args, workDir, "", initialInput, true, env)
	return nil
}
...
func (service *Service) resumeACP(agent *Agent, message string) error {
	...
	stamped := fmt.Sprintf("[%s] > %s", time.Now().Format("15:04:05"), message)
	_ = service.appendAgentLog(agent.ID, stamped)
...
func (service *Service) appendAgentLog(agentID, line string) error {
	...
	service.emitLogLine(agentID, line)
	return nil
}
...
func (runner *Runner) run(svc *Service, agentID, kind, binary string, args []string, workDir, startMsg, initialInput string, appendLog bool, extraEnv []string) {
	...
	logLine := func(line string) {
		...
		writeLogLine(logFile, stamped)
		svc.emitLogLine(agentID, stamped)
	}
	...
	runAttempt := func(binary string, args []string, initialInput string) (streamTurnStats, error, bool) {
	...
	stats, waitErr, retryable := runAttempt(binary, args, initialInput)
	...
	if kind == "claude-code" && retryable && !stats.Succeeded {
		if a, _ := svc.store.GetAgent(agentID); a != nil && a.SessionID != "" {
			fallback := fallbackClaudeModel(a.Model)
			usingFallback := false
			resumeArgs := func(model string) []string {
				_, ra, err := buildResumeCommand(kind, binary, a.SessionID, "", a.Source, model, a.AllowedTools)
				if err != nil {
					return args
				}
				return ra
			...
			for attempt := 1; attempt <= maxAPIRetries && retryable && !stats.Succeeded; attempt++ {
				...
				time.Sleep(delay)
				stats, waitErr, retryable = runAttempt(binary, resumeArgs(model), initialInput)
				if runner.consumeAwaiting(agentID) {
					svc.persistTurnStats(agentID, stats)
					return
				}
...
func (service *Service) emitLogLine(agentID, line string) {
	if service.store == nil {
		return
	}
	service.store.Emit(agentLogEvent, map[string]any{"agentId": agentID, "line": line})
}
...
func (service *Service) RespondToAgentQuestion(agentID, toolUseID, answer string) error {
	...
	// The user has responded: drop the persisted question immediately and
	// unconditionally, before attempting delivery. This guarantees the panel
	// never reappears — on this view, after an agent switch, or after a restart
	// — no matter which delivery path runs below or whether it succeeds. None of
	// the delivery paths read the persisted question (they use toolUseID), so
	// clearing first is safe.
	_ = service.store.PatchAgent(agentID, map[string]any{"pendingQuestion": nil})
	...
	_ = service.appendAgentLog(agentID, fmt.Sprintf("← [#%s] %s", toolUseID, summarizeAnswer(answer)))

	// Flip the dot back to blue while the agent finishes its turn. The resume
	// path leans on Send(), which already patches "working".
	...
	// message. (claude-code can't accept a tool_result on a resumed session —
	// the API rejects it as an assistant-message prefill — so the live-stdin
	// path above is the only one that completes the tool call directly.)
	if err := service.Send(agentID, answer); err != nil {
...
func (service *Service) runnerError(agentID string, logFile io.Writer, msg string) {
	...
	service.emitLogLine(agentID, msg)
...
func (service *Service) markAgentError(agentID, msg string) {
	...
	if err := service.appendAgentLog(agentID, stamped); err != nil {
		service.emitLogLine(agentID, stamped)
	}
...
func (service *Service) markAgentStopped(agentID string) {
	...
	_ = service.appendAgentLog(agentID, fmt.Sprintf("[%s] (stopped by user)", time.Now().Format("15:04:05")))
