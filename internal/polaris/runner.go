...
func (service *Service) Spawn(in SpawnAgentInput) (*Agent, error) {
	...
	if isACP {
		...
		var berr error
		binary, args, berr = buildSpawnCommand(in.Kind, in.Binary, in.Model, sessionID, task, source)
		if berr != nil {
			cleanupWorktree()
			return nil, berr
		}
...
func (service *Service) CreatePRForAgent(agentID string) (string, error) {
	...
	url, err := gh.CreatePullRequest(agent.Worktree.Path, title, body)
	if err != nil {
		return "", err
	}
...
func (runner *Runner) run(svc *Service, agentID, kind, binary string, args []string, workDir, startMsg, initialInput string, appendLog bool, extraEnv []string) {
	...
	if waitErr != nil {
		...
		if dropped := runner.clearPending(agentID); len(dropped) > 0 {
			_ = svc.appendAgentLog(agentID, fmt.Sprintf("(dropped %d queued message(service))", len(dropped)))
		}
		return
	}
	svc.persistTurnStats(agentID, stats)
