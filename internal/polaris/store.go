...
const agentColumns = `id, projectId, kind, source, summary, status, startedAt, pid, sessionId, model, providerId, tokensJson, costUsd, filesModified, toolsUsed, worktreeJson, pendingQuestionJson`
...
func (store *Store) UpsertAgent(a Agent) (Agent, error) {
	...
	_, err = store.db.Exec(
		`INSERT INTO agents (id, projectId, kind, source, summary, status, startedAt, sessionId, model, providerId, tokensJson, costUsd, filesModified, toolsUsed, worktreeJson, pendingQuestionJson) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		 ON CONFLICT(id) DO UPDATE SET projectId=excluded.projectId, kind=excluded.kind, source=excluded.source, summary=excluded.summary, status=excluded.status, startedAt=excluded.startedAt, sessionId=excluded.sessionId, model=excluded.model, providerId=excluded.providerId, tokensJson=excluded.tokensJson, costUsd=excluded.costUsd, filesModified=excluded.filesModified, toolsUsed=excluded.toolsUsed, worktreeJson=excluded.worktreeJson, pendingQuestionJson=excluded.pendingQuestionJson`,
		a.ID, a.ProjectID, a.Kind, a.Source, a.Summary, a.Status, a.StartedAt, a.SessionID, a.Model, a.ProviderID, tokensJSON, a.CostUSD, a.FilesModified, a.ToolsUsed, worktreeJSON, pendingQuestionJSON,
	)
	if err != nil {
		return a, err
	}
...
func scanAgent(r rowScanner) (Agent, error) {
	var a Agent
	var tokensJSON, worktreeJSON, pendingQuestionJSON string
	err := r.Scan(&a.ID, &a.ProjectID, &a.Kind, &a.Source, &a.Summary, &a.Status, &a.StartedAt, &a.PID, &a.SessionID, &a.Model, &a.ProviderID, &tokensJSON, &a.CostUSD, &a.FilesModified, &a.ToolsUsed, &worktreeJSON, &pendingQuestionJSON)
	if err != nil {
		return a, err
	}
	if tokensJSON != "" {
...
func (store *Store) GetCustomProvider(id string) (*CustomProvider, error) {
	var p CustomProvider
	var models string
	err := store.db.QueryRow(`SELECT id, name, color, endpoint, apiKey, apiType, models FROM custom_providers WHERE id = ?`, id).
		Scan(&p.ID, &p.Name, &p.Color, &p.Endpoint, &p.APIKey, &p.APIType, &models)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
	if err != nil {
