...
// usageParts breaks a usage snapshot into its categories so the UI can explain
// why a turn's total is large — cache_read (the fixed context replayed each
// turn) usually dominates the few "new" input/output tokens.
...
func streamClaudeJSON(reader io.Reader, logFile io.Writer, onLine func(string), onAsk onAskUserQuestion, onTok onTokens, onResult func()) streamTurnStats {
	...
	for scanner.Scan() {
		...
		for _, line := range renderStreamEvent(evt, filesSet, toolInputs, onAsk, &stats) {
			emitStreamLine(logFile, onLine, line)
		}
		if onTok != nil && stats.Tokens != prevTokens {
			onTok(stats.Tokens, stats.Parts, stats.CostUSD)
		}
		// Close stdin after the result event so Claude exits cleanly. This is
		// safe because AskUserQuestion tool_results are always delivered before
		// the result event, and no further stdin writes are needed after it.
		if kind, _ := evt["type"].(string); kind == "result" && onResult != nil {
			onResult()
		}
	}
...
func (p usageParts) add(o usageParts) usageParts {
	return usageParts{
		Input:         p.Input + o.Input,
		Output:        p.Output + o.Output,
		CacheCreation: p.CacheCreation + o.CacheCreation,
		CacheRead:     p.CacheRead + o.CacheRead,
	}
}
