...
// tmpWriteRe matches shell redirections that write to /tmp (e.g. "> /tmp/plan.md", "tee /tmp/plan.md").
...
type streamTurnStats struct {
	...
	FilesModified int
	ToolsUsed     int
	// Succeeded is set when a `result` event arrives with is_error false, i.e.
	// the turn completed cleanly. The runner uses it to decide whether a
	// retryable API error seen mid-stream actually broke the turn.
	Succeeded bool
}
...
// tmpPlanFile scans bash tool inputs for a command that writes to /tmp and returns
// one of the matched file paths. Used to recover plan content when ExitPlanMode
// arrives with an empty plan field.
