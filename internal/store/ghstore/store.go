...
type Diff struct {
	...
	PRsOpened     []gh.PullRequest
	PRsClosed     []gh.PullRequest
	PRsUpdated    []gh.PullRequest
	IssuesAdded   []gh.Issue
	IssuesAssigned []IssueAssignment
	RunsAdded     []gh.WorkflowRun
...
func (s *Store) GetPRs(ctx context.Context, owner, repo string) ([]gh.PullRequest, error) {
...
func (s *Store) GetIssues(ctx context.Context, owner, repo string) ([]gh.Issue, error) {
	snap, err := s.GetSnapshot(ctx, owner, repo)
	if err != nil {
		return nil, err
	}
...
func (s *Store) GetRuns(ctx context.Context, owner, repo string) ([]gh.WorkflowRun, error) {
	snap, err := s.GetSnapshot(ctx, owner, repo)
	if err != nil {
		return nil, err
	}
...
func fetchAll(owner, repo string) (Snapshot, error) {
	...
	go func() {
		defer wg.Done()
		prs, err := gh.ListPullRequests(owner, repo)
		mu.Lock()
		if err != nil {
	...
	go func() {
		defer wg.Done()
		issues, err := gh.ListIssues(owner, repo)
		mu.Lock()
		if err != nil {
...
func newPRs(before, after []gh.PullRequest) []gh.PullRequest {
	seen := map[int]struct{}{}
...
func removedPRs(before, after []gh.PullRequest) []gh.PullRequest {
	seen := map[int]struct{}{}
	for _, p := range after {
		seen[p.Number] = struct{}{}
	}
...
func updatedPRs(before, after []gh.PullRequest) []gh.PullRequest {
	byNum := map[int]gh.PullRequest{}
	for _, p := range before {
		byNum[p.Number] = p
	}
	var out []gh.PullRequest
...
func newIssues(before, after []gh.Issue) []gh.Issue {
	seen := map[int]struct{}{}
...
func newRuns(before, after []gh.WorkflowRun) []gh.WorkflowRun {
...
func completedRuns(before, after []gh.WorkflowRun) []gh.WorkflowRun {
	prev := map[int64]gh.WorkflowRun{}
	for _, r := range before {
		prev[r.ID] = r
	}
	var out []gh.WorkflowRun
...
func newAssignments(before, after []gh.Issue) []IssueAssignment {
