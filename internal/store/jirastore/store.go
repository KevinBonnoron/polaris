...
type Store struct {
	...
	// configs maps key → the jira.Config the store should use when fetching.
	// Set on Register and consulted on every refresh.
	configMu sync.RWMutex
	configs  map[string]jira.Config
}
...
func (s *Store) GetSnapshot(ctx context.Context, k Key, cfg jira.Config) (Snapshot, error) {
	s.setConfig(k, cfg)
...
func (s *Store) setConfig(k Key, cfg jira.Config) {
	s.configMu.Lock()
...
func (s *Store) getConfig(k Key) (jira.Config, bool) {
	s.configMu.RLock()
...
func (s *Store) Refresh(ctx context.Context, k Key) error {
	...
	sprint, err := jira.FetchActiveSprint(cfg)
...
func (s *Store) Register(ctx context.Context, k Key, cfg jira.Config, refKey string, interval time.Duration) {
...
func issueMap(snap Snapshot) map[string]jira.Issue {
...
func addedIssues(before, after Snapshot) []jira.Issue {
	prev := issueMap(before)
	var out []jira.Issue
	if after.Sprint == nil {
		return out
...
func assigneeKey(i jira.Issue) string {
	if i.AssigneeEmail != "" {
