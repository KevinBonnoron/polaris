// Package jira fetches sprint board data from Jira Cloud's REST API using
// Basic auth (email + API token). We translate Jira's verbose schema into
// small DTOs so the frontend doesn't depend on Atlassian's full payloads.
package jira
...
var (
	...
	ErrMissingConfig = errors.New("jira: missing baseUrl, email, token or projectKey")
	// ErrNoBoard means no agile board was found for the given project.
	ErrNoBoard = errors.New("jira: no agile board found for project")
	// ErrNoSprint means the project's board has no active sprint.
	ErrNoSprint = errors.New("jira: no active sprint on board")
)
...
func TransitionIssue(cfg Config, issueKey string, targetStatusIDs []string) error {
	...
	if issueKey == "" || len(targetStatusIDs) == 0 {
		return errors.New("jira: issueKey and targetStatusIDs are required")
	}
	base := strings.TrimRight(cfg.BaseURL, "/")
	...
	if len(wanted) == 0 {
		return errors.New("jira: targetStatusIDs are empty")
	}
...
var ErrNoTransition = errors.New("jira: no available transition to that status")
...
func FetchLastComment(cfg Config, issueKey string) (string, error) {
	...
	if issueKey == "" {
		return "", errors.New("jira: issueKey is required")
	}
	base := strings.TrimRight(cfg.BaseURL, "/")
...
func FetchIssueDetail(cfg Config, issueKey string) (*IssueDetail, error) {
	...
	if issueKey == "" {
		return nil, errors.New("jira: issueKey is required")
	}
	base := strings.TrimRight(cfg.BaseURL, "/")
