...
func (runner *Runner) Start(manifestPath, packageManager, script, runEnv string) (string, error) {
	...
	return runner.launch(manifestPath, runEnv, [][]string{{script}})
}
...
// launch runs the given task invocations under a single runID, streaming their
// combined output. Each entry in commands is one `task <args>` call; they run
// one after another and stop early if one fails or the run is cancelled.
...
// runOne runs a single `task <args>` command to completion, streaming its
...
func (runner *Runner) runOne(ctx context.Context, runID, workDir, runEnv string, args []string) (int, string, bool) {
	cmd, err := devenv.BuildCommand(ctx, workDir, runEnv, "task", args)
	if err != nil {
	...
	runner.emitLine(runID, "system", fmt.Sprintf("$ task %s", strings.Join(args, " ")))
