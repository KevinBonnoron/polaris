// Package gh shells out to the `gh` CLI to read repository state without
// asking the user for a token. We deliberately keep the surface small and
// translate `gh`'s JSON into our own DTOs so the frontend doesn't depend on
// GitHub's full schema.
package gh
