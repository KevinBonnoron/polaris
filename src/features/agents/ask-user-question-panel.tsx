...
export function AskUserQuestionPanel({ payload, onSubmit, onCancel }: Props) {
    ...
    <div className="flex w-full shrink-0 flex-col gap-3 rounded-md border border-border bg-muted/30 p-3">
      <div className="flex shrink-0 items-center justify-between gap-3">
        <span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
          {t('agents.askUserQuestion.title', { defaultValue: 'Agent is asking a question' })}
        </span>
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            onClick={onCancel}
            aria-label={t('agents.askUserQuestion.cancel', { defaultValue: 'Cancel' })}
      ...
      <div className="flex flex-col gap-2">
        ...
        {((current.multiSelect && multi[step]?.has(OTHER_KEY)) || (!current.multiSelect && single[step] === OTHER_KEY)) && (
          <Input autoFocus placeholder={t('agents.askUserQuestion.otherPlaceholder', { defaultValue: 'Your answer' })} value={other[step] ?? ''} onChange={(e) => setOther((prev) => ({ ...prev, [step]: e.target.value }))} />
        )}
      </div>
