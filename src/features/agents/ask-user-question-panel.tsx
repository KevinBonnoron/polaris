...
export function AskUserQuestionPanel({ payload, projectPath, onSubmit, onCancel }: Props) {
  ...
  return (
    <div className="grid w-full shrink-0 max-h-[min(70vh,32rem)] grid-rows-[auto_1fr_auto] gap-3 rounded-md border border-border bg-muted/30 p-3">
      <div className="flex shrink-0 items-center justify-between gap-3">
        <span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('agents.askUserQuestion.title')}</span>
        <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              {payload.questions.map((_, idx) => (
                <button
                  ...
                  key={idx}
                  onClick={() => setStep(idx)}
                  aria-label={t('agents.askUserQuestion.goToStep', { n: idx + 1 })}
          ...
          <button type="button" onClick={onCancel} aria-label={t('agents.askUserQuestion.cancel')} className="rounded-sm text-muted-foreground transition-colors hover:text-foreground">
            <X className="size-4" />
          </button>
        </div>
      </div>
      ...
      <ScrollArea className="min-h-0">
        <div className="flex flex-col gap-2 pr-3">
          ...
          <div className="text-sm text-foreground [&_*]:font-sans [&_code]:font-mono">
          ...
          <div className="flex flex-col gap-1.5">
            {optionsWithOther.map((opt) => {
              ...
              return (
                ...
                >
                  <span className="font-medium text-foreground">{isOther ? t('agents.askUserQuestion.other') : opt.label}</span>
      ...
      <div className="flex items-center justify-between gap-2">
        <span className="text-[10px] uppercase tracking-wide text-muted-foreground">{t('agents.askUserQuestion.escToCancel')}</span>
        <div className="flex items-center gap-2">
          {total > 1 && (
            <>
              <Button variant="outline" size="sm" onClick={() => setStep((s) => Math.max(0, s - 1))} disabled={step === 0}>
                <ChevronLeft className="size-4" />
                {t('agents.askUserQuestion.previous')}
              </Button>
