...
export function AgentDraftCard({ agent, selected, onSelect }: Props) {
  ...
  return (
    <div className="group relative">
      <button type="button" onClick={onSelect} className={cn('relative overflow-hidden flex w-full flex-col gap-1 rounded-md border px-3 py-2.5 text-left transition-colors', selected ? 'border-border/60 bg-accent' : 'border-border/30 hover:border-border/50 hover:bg-accent/50')}>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="shrink-0 text-[11px] text-muted-foreground">{t('agents.status.draft')}</span>
        </div>
      ...
      <div className="absolute right-2 top-2 hidden group-hover:block">
        <Button variant="ghost" size="icon" onClick={remove} className="size-6 text-destructive/60 hover:text-destructive" title={t('agents.draft.discard')}>
