...
export function MentionTextarea({ value, onChange, onKeyDown, onAttach, disabled, placeholder, className, autoFocus, inputRef, projectPath, commands, onCommand }: Props) {
  ...
  const [menu, setMenu] = useState<'file' | 'command' | 'command-arg' | null>(null);
  ...
  const commandArgItems = (argCommand?.args ?? []).filter((a) => !query || a.value.toLowerCase().includes(query.toLowerCase()) || a.label.toLowerCase().includes(query.toLowerCase()));
  const items = menu === 'command' ? commandItems : menu === 'command-arg' ? commandArgItems : menu === 'file' ? fileItems : [];
  ...
  const handleChange = (e: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    const argMatch = commands?.length ? textBefore.match(/(?:^|\n)\/(\w+)\s+(\S*)$/) : null;
    const argCmd = argMatch ? (commands ?? []).find((c) => c.name === argMatch[1] && c.args?.length) : undefined;
    ...
    if (slashMatch) {
      ...
      setMenu('command');
    } else if (argCmd && argMatch) {
      setTriggerStart(pos - argMatch[2].length);
      if (argMatch[2] !== query) {
        setSelectedIndex(0);
      }
      setArgCommand(argCmd);
      setQuery(argMatch[2]);
      setMenu('command-arg');
    } else if (atMatch) {
  ...
  const selectCommand = (cmd: SlashCommand) => {
    if (cmd.takesArg) {
      ...
      // A command with a preset list of args opens its argument picker straight
      // away. We set the menu state directly because replaceTrigger writes the
      // value via onChange, which does not run handleChange (the textarea's own
      // onChange), so the menu would otherwise stay closed.
      if (cmd.args?.length) {
        setArgCommand(cmd);
        setQuery('');
  ...
  const selectCommandArg = (arg: SlashCommandArg) => {
    setMenu(null);
    const cmd = argCommand;
    setArgCommand(null);
    onChange('');
    if (cmd) {
      onCommand?.(cmd.name, arg.value);
    }
  };
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (menu !== null) {
      ...
      if (e.key === 'Enter') {
        ...
        if (menu === 'command-arg' && commandArgItems[selectedIndex]) {
          e.preventDefault();
          selectCommandArg(commandArgItems[selectedIndex]);
          return;
        }
      ...
      {menu !== null && (
        <div className="absolute bottom-full left-0 z-50 mb-1 w-full min-w-[320px] overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-md">
          <ScrollArea className="max-h-52" viewportProps={{ className: 'max-h-52' }}>
            {items.length === 0 ? (
              <p className="py-6 text-center text-sm text-muted-foreground">{menu === 'file' ? t('agents.detail.mentionNoFiles') : t('agents.detail.commandNone')}</p>
            ) : menu === 'command' ? (
              <div className="py-1">
              ...
              <div className="py-1">
                {commandArgItems.map((arg, i) => (
                  <button
                    ...
                    onMouseDown={(e) => {
                      e.preventDefault();
                      selectCommandArg(arg);
                    }}
                  >
                    <Terminal className={cn('size-3.5 shrink-0', arg.dimmed ? 'text-muted-foreground/50' : 'text-muted-foreground')} />
                    <span className="min-w-0 flex-1">
                      <span className={cn('block truncate text-xs', arg.dimmed && 'text-muted-foreground')}>{arg.label}</span>
                      {arg.description && <span className="block truncate text-[10px] text-muted-foreground/70">{arg.description}</span>}
                    </span>
                    {!arg.description && arg.label !== arg.value && <span className="ml-auto shrink-0 font-mono text-xs text-muted-foreground/60">{arg.value.length > 8 ? arg.value.slice(0, 8) : arg.value}</span>}
                  </button>
                ))}
              </div>
            ) : (
