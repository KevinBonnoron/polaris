...
const STATUS_ORDER = ['draft', 'waiting', 'error', 'working', 'completed', 'idle'] as const;
...
export function AgentsPage() {
  ...
  const groups = useMemo(() => {
    ...
    return STATUS_ORDER.map((status) => ({ status, items: toShow.filter((a) => a.status === status) })).filter(
      (g) => g.items.length > 0,
    );
          ...
          <div className="flex w-80 flex-col gap-1 pr-2">
              ...
              <div className="mb-1 flex flex-wrap gap-1 px-1">
                <button
                  ...
                  className={cn(
                    ...
                    effectiveFilter === null ? 'bg-foreground/10 font-medium text-foreground' : 'text-muted-foreground hover:text-foreground',
                  )}
                >
                ...
                {activeStatuses.map((status) => (
                  <button
                    key={status}
                    onClick={() => setStatusFilter(effectiveFilter === status ? null : status)}
                    className={cn(
                      'rounded px-2 py-0.5 text-xs transition-colors',
                      effectiveFilter === status ? 'bg-foreground/10 font-medium text-foreground' : 'text-muted-foreground hover:text-foreground',
                    )}
                  >
            ...
            {groups.map((group, i) => (
              <div key={group.status}>
                {groups.length > 1 && (
                  <div className={cn('px-1 pb-1 text-xs text-muted-foreground/60', i > 0 && 'mt-3 border-t border-border/40 pt-2')}>
                    {t(`agents.status.${group.status}`)}
                  </div>
                )}
