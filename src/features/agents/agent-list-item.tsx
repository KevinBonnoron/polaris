import { Copy, ExternalLink, GitBranch, GitPullRequest, Pencil, Square, Trash2 } from 'lucide-react';
...
export function AgentListItem({ agent, selected, onSelect, providerIcon }: Props) {
  ...
  return (
    <div className="group relative">
      {isWorking && <span className="pointer-events-none absolute inset-0 animate-pulse rounded-md ring-1 ring-inset ring-primary/50" aria-hidden />}
      <div
        role="button"
        tabIndex={0}
        ...
        className={cn('flex w-full cursor-pointer flex-col gap-1 rounded-md border px-3 py-2.5 text-left transition-colors', selected ? 'border-border/60 bg-accent' : 'border-border/30 hover:border-border/50 hover:bg-accent/50', isEditing && 'cursor-default')}
      >
        <div className="flex items-center gap-2">
          ...
          {!isEditing && (
            <div
              className="ml-auto flex items-center gap-0.5 opacity-0 transition-opacity pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto"
