      ...
      try {
        const input = provider
          ...
          : cliCfg?.id === 'opencode'
            ...
            : cliCfg
              ? { projectId: pid, kind: cliCfg.id, task: text, model: spawnModel, binary: cliCfg.binary, isolated, allowedTools: allowedTools.length > 0 ? allowedTools : undefined }
              : null;
            ...
            <div className="mb-2 flex flex-wrap gap-2">
              {attachments.map((att, i) => (
                <div key={att.path} className="group relative">
                  {att.preview ? (
                    <img src={att.preview} alt="" className="h-20 rounded-md border object-cover" />
                  ) : (
                    <div className="flex h-20 w-24 items-center justify-center rounded-md border bg-muted text-xs text-muted-foreground">{att.path.split('/').pop()}</div>
                  )}
                  <button
                    type="button"
          ...
          <div className="flex items-start gap-2">
            ...
            {speechRecognitionAvailable && (
              <Button
                ...
                size="icon"
                title={isRecording ? t('agents.detail.stopRecording') : t('agents.detail.recordVoice')}
            ...
            {message.trim() || attachments.length > 0 || !isWorking ? (
              <Button
                size="icon"
                title={t('agents.detail.send')}
                disabled={(!isDraft && !!sendDisabledReason) || sending || (!message.trim() && attachments.length === 0) || (isDraft && !spawnModel)}
                onClick={() => void send()}
                className="size-10"
              >
