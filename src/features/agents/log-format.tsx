...
export function buildLogBlocks(lines: string[]): LogBlock[] {
  ...
  const flushToolResult = () => {
    if (toolResultLines.length > 0 && toolResultKey !== null) {
      blocks.push({ type: 'tool-result', lines: toolResultLines, key: toolResultKey, toolId: toolResultCurrentId ?? undefined });
      toolResultLines = [];
      toolResultKey = null;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (rest.startsWith('(thinking) ')) {
      ...
      const { id, rest: body } = extractToolId(rest.slice('← '.length));
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (block.type === 'line' && block.rest.startsWith('→ ')) {
      ...
      if (call) {
        attachResult(call, 'success', block.lines.map(stripXmlTags));
        absorbed.add(i);
      }
...
function TodoResultPanel({ lines }: { lines: string[] }) {
  return (
    <div className="mt-1 max-h-64 overflow-auto rounded border border-border/60 bg-muted/30 px-2 py-1.5 text-muted-foreground/80">
      <div className="flex flex-col gap-0.5 text-[11px] leading-relaxed">
        {lines.map((l, i) => {
          const m = l.match(TODO_RE);
          if (!m) return <span key={i}>{l}</span>;
          const marker = m[1];
...
function ToolResultPanel({ lines, toolName }: { lines: string[]; toolName?: string }) {
  ...
  if (isTodo) return <TodoResultPanel lines={lines} />;
  return (
    <div className="mt-1 max-h-64 overflow-auto rounded border border-border/60 bg-muted/30 px-2 py-1.5 text-muted-foreground/80">
...
const mdComponents: React.ComponentProps<typeof ReactMarkdown>['components'] = {
  ...
  pre: ({ children }) => <pre className="mb-2 overflow-x-auto rounded bg-[#0d1117] p-3 font-mono text-xs last:mb-0">{children}</pre>,
