...
export function FilesTab({ projectPath }: FilesTabProps) {
        ...
        Promise.all(
          nodes.map(async (n) => {
            if (n.entry.path === path) {
              if (n.loaded) { return { ...n, expanded: !n.expanded }; }
              try {
            ...
            if (n.children) { return { ...n, children: await load(n.children) }; }
            return n;
          }),
        );
  ...
  const extensions = useMemo(
    () => [getLanguageExtension(selectedPath ?? ''), sizeTheme],
    [selectedPath],
  );
  ...
  return (
    <>
      ...
      {node.entry.isDir && node.expanded && node.children && (
        <NodeList nodes={node.children} depth={depth + 1} selectedPath={selectedPath} onExpand={onExpand} onSelect={onSelect} />
      )}
    </>
  );
}
