...
import { Activity, Boxes, Bug, Cloud, Container, GitBranch, KanbanSquare, Layout, Mail, MessageSquare, Package, Rocket } from 'lucide-react';
...
async function detectNode(projectPath: string): Promise<IntegrationConfig | IntegrationConfig[] | null> {
  ...
  const configs = projects.map((p) => ({ manifestPath: p.manifestPath, packageManager: p.packageManager }));
