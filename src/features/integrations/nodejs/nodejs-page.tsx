import { AlertTriangle, ArrowUpCircle, ChevronDown, ChevronRight, Download, Package, Play, Plug, Settings, ShieldAlert, Square, Trash2, Unplug } from 'lucide-react';
...
export function NodejsPage() {
  ...
  const { run, isRunning, config, startScript, runCommand, runInWorkspaces, stop } = useNodejsRun();
                ...
                <Tabs value={tab} onValueChange={setTab}>
                  <TabsList className="flex h-auto flex-wrap justify-start">
                    <TabsTrigger value="all" className="text-xs">
                      {t('integrations.nodejs.viewAll')}
                    </TabsTrigger>
                    {workspaces.map((ws) => (
                      <TabsTrigger key={ws.manifest} value={ws.manifest} className="text-xs">
