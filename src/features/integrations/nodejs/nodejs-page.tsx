import { AlertTriangle, ArrowUpCircle, ChevronDown, ChevronRight, Download, FlaskConical, Hammer, Package, Play, Plug, Settings, ShieldAlert, Square, Stethoscope, Trash2, Unplug } from 'lucide-react';
...
export function NodejsPage() {
  ...
  const extraScripts = useMemo(
    () => (config?.extraScripts ?? '').split(',').map((s) => s.trim()).filter(Boolean),
    [config?.extraScripts],
  );
  ...
  const refreshAll = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [p, w] = await Promise.all([ListNodePackages(manifestPath), ListNodeWorkspaces(manifestPath)]);
      setPackages(p ?? []);
      setWorkspaces(w ?? []);
    } catch (err) {
            ...
            {(
              ...
              return (
                <button
                  ...
                  className={cn(
                    'group flex flex-col gap-1 rounded-md border px-3 py-2.5 text-left transition-colors disabled:pointer-events-none disabled:opacity-50',
                    active ? 'border-primary/40 bg-primary/5' : 'hover:bg-muted/50',
                  )}
                >
                  <div className="flex items-center gap-2">
                    {active ? (
                      <Square className="size-3 shrink-0 animate-pulse text-primary" />
                    ) : (
                      <Icon className="size-3 shrink-0 text-muted-foreground transition-colors group-hover:text-foreground" />
                    )}
              ...
              {extraScripts.map((name) => {
                ...
                return (
                  <Button
                    ...
                    size="sm"
                    className={cn('gap-1.5 font-mono text-xs', active && 'animate-pulse')}
                    disabled={isRunning && !active}
                    onClick={() => (active ? stop() : startScript(name))}
                  >
