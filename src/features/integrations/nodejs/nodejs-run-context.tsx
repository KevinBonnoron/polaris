...
export function NodejsRunProvider({ children }: { children: ReactNode }) {
  ...
  const config = (project ? (getIntegrations(project).nodejs as NodejsConfig | undefined) : undefined) ?? null;
  ...
  return <NodejsRunContext.Provider value={{ run, isRunning, config, terminalOpen, setTerminalOpen, startScript, runCommand, runInWorkspaces, stop, clear }}>{children}</NodejsRunContext.Provider>;
}
