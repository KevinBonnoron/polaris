...
import { CreateJiraIssue, ListJiraIssueTypes } from '@/wailsjs/go/main/App';
...
export function CreateJiraIssueModal({ config, onCreated, children }: PropsWithChildren<Props>) {
  ...
  const [types, setTypes] = useState<jira.IssueType[]>([]);
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        const input = jira.CreateIssueInput.createFrom({ summary: value.summary.trim(), issueTypeId: value.typeId });
        const key = await CreateJiraIssue(cfg, input);
