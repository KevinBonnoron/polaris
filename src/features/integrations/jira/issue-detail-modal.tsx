...
export function JiraIssueDetailModal({ config, issueKey, children, ...modeProps }: PropsWithChildren<Props>) {
  ...
  const [detail, setDetail] = useState<jira.IssueDetail | null>(null);
  const [comments, setComments] = useState<jira.Comment[] | null>(null);
  const [history, setHistory] = useState<jira.HistoryEntry[] | null>(null);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const cfg = jira.Config.createFrom(config);
...
function ChangeLine({ change }: { change: jira.HistoryChange }) {
  return (
...
function DetailBody({ detail, formatDate }: { detail: jira.IssueDetail; formatDate: (unix: number) => string }) {
