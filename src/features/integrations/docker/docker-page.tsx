...
import { imageRef } from '@/collections/docker.collection';
...
export function DockerPage() {
  ...
  const instances = getInstances(project, 'docker');
  const [selectedIndex, setSelectedIndex] = useState(0);
  const integration = findIntegration('docker')!;

  useEffect(() => setSelectedIndex(0), [project?.id]);

  const config = instances[selectedIndex] as DockerConfig | undefined;
  ...
  return <DockerView projectId={project.id} dockerfilePath={dockerfilePath} integration={integration} instances={instances} selectedIndex={selectedIndex} onSelectInstance={setSelectedIndex} projectPath={project.path ?? ''} />;
}

interface DockerViewProps {
  projectId: string;
  dockerfilePath: string;
  integration: ReturnType<typeof findIntegration> & {};
  instances: ReturnType<typeof getInstances>;
  selectedIndex: number;
  onSelectInstance: (i: number) => void;
  projectPath: string;
}

function DockerView({ projectId, dockerfilePath, integration, instances, selectedIndex, onSelectInstance, projectPath }: DockerViewProps) {
  const { t } = useTranslation();
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <InstanceSelector integration={integration} instances={instances} selectedIndex={selectedIndex} onSelect={onSelectInstance} projectPath={projectPath} />
            <RefreshAction onRefresh={() => reload()} loading={loading} />
            <ConfigureIntegrationModal projectId={projectId} integrationId="docker" instanceIndex={selectedIndex}>
