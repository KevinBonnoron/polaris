...
import { AlertTriangle, BellRing, Bot, Check, CheckCheck, MessageCircleQuestion, X } from 'lucide-react';
...
const SEVERITY_TONE: Record<NonNullable<Notification['severity']>, string> = {
...
function toneFor(n: Notification): string {
  ...
  return SEVERITY_TONE[n.severity] ?? SEVERITY_TONE.info;
}
...
export function NotificationPopoverContent() {
  ...
  return (
    <div className="flex flex-col gap-3" role="dialog" aria-label="Notifications">
      ...
      {openAgent && <AgentDetailModal agentId={openAgent} open={true} onOpenChange={(o) => !o && setOpenAgent(null)} />}
    </div>
  );
}
