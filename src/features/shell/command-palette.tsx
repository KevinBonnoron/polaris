...
type Launch = { kind: 'newAgent'; agentKind: AgentKind } | { kind: 'addProject' } | { kind: 'addIntegration' } | { kind: 'settings' };
...
export function CommandPalette({ open, onOpenChange }: Props) {
  ...
  return (
    <>
            ...
            <CommandList>
              ...
              <CommandGroup heading={t('commandPalette.groups.actions')}>
                {installedKinds.map((k, idx) => {
                  const Icon = k.icon;
                  return (
                    <CommandItem key={k.id} value={`new agent ${k.label}`} onSelect={run({ kind: 'newAgent', agentKind: k.id })}>
                      <Icon />
                      <span>
      ...
      {launch?.kind === 'newAgent' && <AgentDetailModal pending={{ kindId: launch.agentKind }} open={true} onOpenChange={dismissLaunch} />}
