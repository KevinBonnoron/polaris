...
import { Check, ChevronDown, ChevronRight, FileEdit, FileX, ListTree, Plus, Rows3, Undo2 } from 'lucide-react';
...
export function GitChangesPanel({ ops, pollInterval = 0, headerSlot, resetKey }: Props) {
  ...
  return (
    ...
    <div className="flex h-full flex-col gap-3" onKeyDown={onContainerKeyDown}>
      ...
      {(statuses ?? []).length === 0 ? (
        ...
        <div className="flex h-0 flex-1 gap-3">
          <div className="flex h-full w-72 shrink-0 flex-col overflow-hidden rounded-md border border-border">
              ...
              <div className="flex flex-col gap-0.5 p-1">
                ...
                {!approvedCollapsed &&
                  (approved.length === 0 ? (
                    ...
                    <TreeView node={buildTree(approved)} depth={0} selected={selected} onSelect={setSelected} onUnstage={(p) => void unstageFile(p)} busy={busy} t={t} showStage={false} />
                  ) : (
                ...
                {!pendingCollapsed &&
                  (view === 'tree' ? (
                    <TreeView node={buildTree(pending)} depth={0} selected={selected} onSelect={setSelected} onStage={(p) => void stageFile(p)} busy={busy} t={t} showStage={true} />
                  ) : (
                    pending.map((f) => <FileRow key={f.path} file={f} selected={selected === f.path} onSelect={() => setSelected(f.path)} onStage={() => void stageFile(f.path)} busy={busy} t={t} />)
                  ))}
              </div>
            </ScrollArea>

            <div className="flex shrink-0 flex-col gap-1.5 border-t border-border p-1.5">
              <Textarea value={commitMessage} onChange={(e) => setCommitMessage(e.target.value)} placeholder={t('agents.detail.commitMessagePlaceholder')} className="min-h-14 resize-none text-xs" />
              {isProtected && <p className="px-1 text-[10px] text-amber-400">{t('agents.detail.protectedBranchWarn', { branch: gitState?.branch ?? '' })}</p>}
              <div className="flex">
                <Button type="button" size="sm" onClick={() => void runShip('ship')} disabled={!canShip} className="h-8 flex-1 rounded-r-none">
                  <Check className="size-3.5" />
                  {t('agents.detail.ship')}
                </Button>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="min-w-44">
                    ...
                    <DropdownMenuItem onSelect={() => void runShip('push-only')} disabled={aheadCount === 0}>
                      {t('agents.detail.pushOnly')}
                    </DropdownMenuItem>
                    <DropdownMenuItem onSelect={() => void runShip('push-with-lease')} disabled={aheadCount === 0}>
                      {t('agents.detail.pushWithLease')}
                    </DropdownMenuItem>
          ...
          <ScrollArea className="h-full flex-1">
            <div className="pr-3">
              {selectedDiff ? (
                <DiffPanel file={selectedDiff} />
              ) : (
                <div className="flex h-full flex-col items-center justify-center gap-2 rounded-md border border-dashed border-border p-8 text-center text-xs text-muted-foreground">
...
function FileRow({ file, selected, onSelect, onStage, onUnstage, busy, t, indent = 0 }: { file: FileStatus; selected: boolean; onSelect: () => void; onStage?: () => void; onUnstage?: () => void; busy: boolean; t: (key: string) => string; indent?: number }) {
...
function TreeView({
  ...
  const walk = (n: TreeNode, level: number) => {
    ...
    for (const child of children) {
      ...
      if (isDir) {
        rows.push(
          <button key={`d:${child.fullPath}`} type="button" onClick={() => setOpen((prev) => ({ ...prev, [child.fullPath]: !isOpen }))} style={{ paddingLeft: `${0.5 + level * 0.75}rem` }} className="flex w-full items-center gap-1 rounded-sm py-1 pr-1.5 text-left text-xs text-muted-foreground hover:bg-muted/60">
        ...
        const f = child.file;
        rows.push(<FileRow key={`f:${f.path}`} file={f} selected={selected === f.path} onSelect={() => onSelect(f.path)} onStage={showStage && onStage ? () => onStage(f.path) : undefined} onUnstage={!showStage && onUnstage ? () => onUnstage(f.path) : undefined} busy={busy} t={t} indent={level + 1.5} />);
      }
    }
  };
  walk(node, depth);
...
function DiffPanel({ file }: { file: DiffFile }) {
  const lang = getLang(file.path);
  return (
    <div className="overflow-hidden rounded-md border border-border">
      <div className="sticky top-0 bg-muted/60 px-3 py-1.5 font-mono text-xs text-foreground">{file.path}</div>
      {file.hunks.map((hunk) => {
            ...
            <div>
              {hunk.lines.map((line, li) => {
                ...
                return (
                  ...
                  <div key={li} className={cn('flex', line.type === '+' && 'bg-emerald-950/40 text-emerald-300', line.type === '-' && 'bg-red-950/40 text-red-300', line.type !== '+' && line.type !== '-' && 'text-muted-foreground')}>
                    <span className="w-9 shrink-0 select-none text-right font-mono text-[10px] leading-[1.375rem] opacity-30">{ol ?? ''}</span>
                    <span className="w-9 shrink-0 select-none text-right font-mono text-[10px] leading-[1.375rem] opacity-30">{nl ?? ''}</span>
                    <span className="w-5 shrink-0 select-none text-center font-mono text-xs opacity-50">{line.type === '+' || line.type === '-' ? line.type : ' '}</span>
                    {/* biome-ignore lint/security/noDangerouslySetInnerHtml: highlight.js output from local git diff */}
                    <span className="whitespace-pre-wrap break-all font-mono text-xs" dangerouslySetInnerHTML={{ __html: highlighted[li] ?? '' }} />
                  </div>
                );
              })}
            </div>
