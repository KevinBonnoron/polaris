          ...
          <div style={{ width: listWidth }} className="flex h-full shrink-0 flex-col overflow-hidden rounded-md border border-border">
            ...
            <ScrollArea className="h-0 flex-1">
              <div className="flex flex-col gap-0.5 p-1">
                ...
                {!pendingCollapsed &&
                  (view === 'tree' ? (
                    <TreeView node={buildTree(pending)} depth={0} selected={selected} onSelect={setSelected} onStage={(p) => void stageFile(p)} onStageDir={(n) => void stageDir(n)} busy={busy} t={t} showStage={true} />
                  ) : (
...
function DiffPanel({ file }: { file: DiffFile }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden rounded-md border border-border">
      <div className="shrink-0 border-b border-border bg-muted/60 px-3 py-1.5 font-mono text-xs text-foreground">{file.path}</div>
      <ScrollArea className="h-0 flex-1">
        {file.hunks.map((hunk) => {
