...
function AgentKindCard({ kind, detecting, defaultModel, onDefaultModelChange }: AgentKindCardProps) {
      ...
      <CardContent className="py-2.5">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="truncate font-mono text-[10px] text-muted-foreground" title={binaryHint}>
              {binaryHint}
            </div>
          </div>
          <AgentModelSelect models={kind.models} value={defaultModel} onChange={onDefaultModelChange} disabled={!installed} />
        </div>
        {isClaude && installed && <ClaudeUsageBar />}
      </CardContent>
    </Card>
  );
}
...
export function AgentsSettings() {
  const { t } = useTranslation();
  const { kinds, loading: detecting } = useAgentClis();
  const [defaultModels, setDefaultModels] = useState<Record<AgentKind, string>>(() => Object.fromEntries(kinds.map((k) => [k.id, k.models[0]?.value])) as Record<AgentKind, string>);
  ...
  return (
    <section className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2">
        {kinds.map((kind) => (
          <AgentKindCard key={kind.id} kind={kind} detecting={detecting} defaultModel={defaultModels[kind.id as AgentKind]} onDefaultModelChange={(model) => setDefaultModels((prev) => ({ ...prev, [kind.id]: model }))} />
        ))}
      </div>
