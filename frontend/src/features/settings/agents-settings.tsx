      ...
      <div className="flex flex-col gap-2">
        ...
        {notInstalled.length > 0 && (
          <>
            ...
            {showNotInstalled && notInstalled.map((kind) => <AgentKindCard key={kind.id} kind={kind} detecting={false} defaultModel={defaults.get(kind.id) ?? kind.models[0]?.value ?? ''} onDefaultModelChange={(model) => defaults.set(kind.id, model)} />)}
          </>
        )}
      </div>
