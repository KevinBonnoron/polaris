...
const prCache = new Map<string, GhEntry<repository.PullRequest>>();
const issueCache = new Map<string, GhEntry<repository.Issue>>();
const runsCache = new Map<string, GhEntry<repository.WorkflowRun>>();
const branchCache = new Map<string, GhEntry<Branch>>();

const repoKey = (owner: string, repo: string) => `${owner}/${repo}`;

export function getPullRequestsEntry(owner: string, repo: string): GhEntry<repository.PullRequest> {
  const key = repoKey(owner, repo);
  const cached = prCache.get(key);
  if (cached) {
    return cached;
  }
  const entry = createGhEntry<repository.PullRequest>({
    getKey: (p) => String(p.number),
    list: async () => singlePage((await ListRepoPullRequests(owner, repo)) as repository.PullRequest[]),
  });
  prCache.set(key, entry);
  return entry;
}

export function getIssuesEntry(owner: string, repo: string): GhEntry<repository.Issue> {
  const key = repoKey(owner, repo);
  const cached = issueCache.get(key);
  if (cached) {
    return cached;
  }
  const entry = createGhEntry<repository.Issue>({
    getKey: (i) => String(i.number),
    list: async () => singlePage((await ListRepoIssues(owner, repo)) as repository.Issue[]),
  });
  issueCache.set(key, entry);
  return entry;
}

export function getWorkflowRunsEntry(owner: string, repo: string): GhEntry<repository.WorkflowRun> {
  const key = repoKey(owner, repo);
  const cached = runsCache.get(key);
  if (cached) {
    return cached;
  }
  const entry = createGhEntry<repository.WorkflowRun>({
    getKey: (r) => String(r.id),
    list: async (page) => {
      const result = await ListRepoWorkflowRuns(owner, repo, page);
      return { items: (result?.runs ?? []) as repository.WorkflowRun[], hasMore: Boolean(result?.hasMore) };
    },
  });
